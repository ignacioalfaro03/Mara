...
export default async function CreatorHomePage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <main className={styles.shell}><div className={styles.container}>
    ...
    <section className={styles.grid}>
      ...
      <article className={styles.third}><p className={styles.eyebrow}>FULFILLMENT</p><p className={styles.metric}>{pending.length}</p><p className={styles.muted}>compras pagadas pendientes</p></article>
      <article className={styles.third}><p className={styles.eyebrow}>DEMAND</p><p className={styles.metric}>{dashboard.opportunities.length}</p><p className={styles.muted}>oportunidades reales</p></article>
    </section>
    ...
    <section className={styles.grid}>
      <article className={styles.card}>
        <p className={styles.eyebrow}>PRIMARY CREATOR SITE</p>
        ...
        {!sites.length ? <form className={styles.form} method="post" action="/api/creator/worlds"><input type="hidden" name="returnTo" value="/creator" /><label>Nombre público<input name="displayName" minLength={2} maxLength={80} required placeholder="Tu nombre o marca" /></label><label>Handle<input name="slug" required placeholder="tu-nombre" /></label><label>Bio<textarea name="description" maxLength={1200} placeholder="Qué encontrará tu audiencia aquí." /></label><label>Visibilidad<select name="visibility"><option value="public">Público al publicar</option><option value="private">Privado</option></select></label><button className={styles.button} type="submit">Crear borrador</button></form> : null}
      </article>

      <article className={styles.card}>
        <p className={styles.eyebrow}>SITE STATUS</p>
    ...
    {selectedSite && config ? <section className={styles.grid}>
      <article className={`${styles.card} ${styles.wide}`}>
        <p className={styles.eyebrow}>EDIT SITE</p><h2>Identidad, branding y módulos.</h2>
        <form className={styles.form} method="post" action={`/api/creator/sites/${selectedSite.id}`}>
          ...
          <div className={styles.twoCol}><label>Avatar URL<input name="avatarUrl" type="url" defaultValue={config.avatarUrl ?? ""} placeholder="https://..." /></label><label>Portada URL<input name="coverUrl" type="url" defaultValue={config.coverUrl ?? ""} placeholder="https://..." /></label></div>
          <div className={styles.twoCol}><label>Color de acento<input name="accent" type="text" defaultValue={config.accent} pattern="#[0-9A-Fa-f]{6}" /></label><label>Visibilidad<select name="visibility" defaultValue={selectedSite.visibility}><option value="public">Público</option><option value="private">Privado</option></select></label></div>
