  ...
  return (
    <main className={styles.shell}><div className={styles.container}>
      ...
      <header className={styles.hero}>
        <p className={styles.eyebrow}>CLIENTE · SOLO TU RELACIÓN</p>
        <h1>{summary.alias || "Cliente Mara"}</h1>
        <p>Qué ha comprado, qué te ha mostrado que le interesa y qué conviene hacer ahora. No incluye identidad legal, compras de otras creadoras ni señales privadas ajenas a tu World.</p>
      </header>
      ...
      <section className={styles.grid}>
        ...
        <article className={styles.card}>
          <p className={styles.eyebrow}>DEBILIDAD · PRIVADA</p>
          <h2>Lo que no quieres olvidar.</h2>
          {crmEnabled ? (
            <form className={styles.form} method="post" action={`/api/creator/customers/${userId}/context`}>
              <input type="hidden" name="returnTo" value={`/creator/customers/${userId}`} />
              <label>Debilidad<textarea name="weaknessNote" maxLength={2000} defaultValue={privateContext?.weakness_note ?? ""} placeholder="Ej. Le encantan los audios personalizados; suele comprar cerca del fin de semana." /></label>
              <p className={styles.small}>Solo tú puedes verla. Nunca aparece para el cliente ni para otra creadora.</p>
              <button className={styles.secondary} type="submit">Guardar</button>
            </form>
          ) : <p className={styles.empty}>El campo privado queda oculto hasta activar la migración/flag CRM en este entorno.</p>}
        </article>
      </section>

      <section className={styles.grid}>
        <article className={styles.card}>
          <p className={styles.eyebrow}>QUÉ LE INTERESA</p>
          <h2>Declarado y visible para ti</h2>
        ...
        <article className={styles.card}>
          <p className={styles.eyebrow}>QUÉ HA PEDIDO</p>
          <h2>Demanda visible en tu relación</h2>
      ...
      <h2 className={styles.sectionTitle}>Compras contigo</h2>
