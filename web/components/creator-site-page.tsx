...
export default async function CreatorSitePage({ params, canonicalPath }: { params: Promise<{ slug: string }>; canonicalPath?: string }) {
  ...
  const siteConfig = creatorSiteConfig(world);
  ...
  const sameAs = Object.values(siteConfig.socials).filter((value): value is string => Boolean(value));
  const structuredData = world.status === "active" && world.visibility === "public"
    ? {
        ...
        url: returnTo,
        image: siteConfig.avatarUrl ?? siteConfig.coverUrl ?? undefined,
        sameAs,
      }
    : null;
  ...
  return (
    <main className={`${styles.shell} ${siteConfig.theme === "bold" ? styles.themeBold : siteConfig.theme === "dark" ? styles.themeDark : styles.themeClean}`}>
      ...
      <div className={styles.container}>
        ...
        {owner && (world.status !== "active" || world.visibility !== "public") ? <p className={styles.notice}>Vista de creadora: este sitio todavía no es público para tu audiencia.</p> : null}
        <header className={`${styles.hero} ${styles.creatorSiteHero}`} style={{ borderColor: siteConfig.accent }}>
          {siteConfig.coverUrl ? <img className={styles.creatorCover} src={siteConfig.coverUrl} alt="" /> : null}
          <div className={styles.creatorIdentity}>
            {siteConfig.avatarUrl ? <img className={styles.creatorAvatar} src={siteConfig.avatarUrl} alt={world.display_name} /> : <div className={styles.creatorAvatarFallback} aria-hidden="true">{world.display_name.slice(0, 1).toUpperCase()}</div>}
            <div>
              <p className={styles.eyebrow}>SITIO EN MARA</p>
              <h1>{world.display_name}</h1>
              <p>{world.description || "Ofertas, ideas y formas de participar directamente con esta creadora."}</p>
              <div className={styles.actions}>
                <a className={styles.button} href={siteConfig.primaryCtaUrl ?? "#offers"} rel={siteConfig.primaryCtaUrl ? "noopener noreferrer" : undefined}>{siteConfig.primaryCtaLabel}</a>
                {siteConfig.socials.instagram ? <a className={styles.secondary} href={siteConfig.socials.instagram} rel="noopener noreferrer">Instagram</a> : null}
                {siteConfig.socials.tiktok ? <a className={styles.secondary} href={siteConfig.socials.tiktok} rel="noopener noreferrer">TikTok</a> : null}
                {siteConfig.socials.x ? <a className={styles.secondary} href={siteConfig.socials.x} rel="noopener noreferrer">X</a> : null}
                {siteConfig.socials.youtube ? <a className={styles.secondary} href={siteConfig.socials.youtube} rel="noopener noreferrer">YouTube</a> : null}
              </div>
            </div>
          </div>
        </header>

        {siteConfig.modules.memory ? <>
        {history.length > 0 ? (
        ...
        {siteConfig.modules.demand ? <>
        <h2 className={styles.sectionTitle}>Qué quiere la audiencia</h2>
        <section className={styles.grid}>
        ...
        {siteConfig.modules.offers ? <>
        <h2 className={styles.sectionTitle} id="offers">Puedes desbloquear</h2>
        <section className={styles.grid}>
        ...
        <p className={styles.sitePowered}>Powered by Mara · Creator Site</p>
      </div>
    </main>
  );
}
